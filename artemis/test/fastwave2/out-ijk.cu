	...
	for (int r0_k2 = 0, k2 = k; r0_k2 < 2; r0_k2++, k2+=blockDim.z) {
		...
		for (int r0_j2 = 0, j2 = j; r0_j2 < 2; r0_j2++, j2+=blockDim.y) {
			...
			for (int r0_i2 = 0, i2 = i; r0_i2 < 2; r0_i2++, i2+=blockDim.x) {
				if (k2 >= k0 && k2 <= min(k0+2*blockDim.z-1, L-2) && j2 >= j0 && j2 <= min(j0+2*blockDim.y-1, M-2) && i2 >= i0 && i2 <= min(i0+2*blockDim.x-1, N-1)) {
					ppgu[k2][j2][i2] = (ppuv[k2][j2+1][i2] - ppuv[k2][j2][i2]) + (ppgc[k2][j2+1][i2] + ppgc[k2][j2][i2]) * 0.5 * ((hhl[k2+1][j2][i2] + hhl[k2][j2][i2]) - (hhl[k2+1][j2+1][i2] + hhl[k2][j2+1][i2])) / ((hhl[k2+1][j2][i2] - hhl[k2][j2][i2]) + (hhl[k2+1][j2+1][i2] + hhl[k2][j2+1][i2]));
				}
			}
		}
	}
