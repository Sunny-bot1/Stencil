...
__global__ void fastwave (double * __restrict__ d_ppgu, double * __restrict__ d_ppuv, double * __restrict__ d_ppgc, double * __restrict__ d_hhl, int L, int M, int N) {
	...
	for (int r0_k2 = 0, k2 = k; r0_k2 < 2; r0_k2++, k2+=blockDim.z) {
		if (k2 >= k0 && k2 <= min(k0+2*blockDim.z-1, L-2) && j >= j0 && j <= min(j0+blockDim.y-1, M-1) && i >= i0 && i <= min(i0+blockDim.x-1, N-2)) {
			ppgu[k2][j][i] = (ppuv[k2][j][i+1] - ppuv[k2][j][i]) + (ppgc[k2][j][i+1] + ppgc[k2][j][i]) * 0.5 * ((hhl[k2+1][j][i] + hhl[k2][j][i]) - (hhl[k2+1][j][i+1] + hhl[k2][j][i+1])) / ((hhl[k2+1][j][i] - hhl[k2][j][i]) + (hhl[k2+1][j][i+1] + hhl[k2][j][i+1]));
		}
	}
}
...
extern "C" void host_code (double *h_ppgu, double *h_ppuv, double *h_ppgc, double *h_hhl, int L, int M, int N) {
	...
	dim3 gridconfig_1 (ceil (N - 1 - 0 + 1, blockconfig_1.x), ceil (M - 1 - 0 + 1, blockconfig_1.y), ceil (L - 1 - 0 + 1, 2*blockconfig_1.z));
