...
__global__ void fastwave (double * __restrict__ d_ppgu, double * __restrict__ d_ppuv, double * __restrict__ d_ppgc, double * __restrict__ d_hhl, int L, int M, int N) {
	...
	for (int r0_i2 = 0, i2 = i; r0_i2 < 2; r0_i2++, i2+=blockDim.x) {
		if (k >= k0 && k <= min(k0+blockDim.z-1, L-2) && j >= j0 && j <= min(j0+blockDim.y-1, M-1) && i2 >= i0 && i2 <= min(i0+2*blockDim.x-1, N-2)) {
			ppgu[k][j][i2] = (ppuv[k][j][i2+1] - ppuv[k][j][i2]) + (ppgc[k][j][i2+1] + ppgc[k][j][i2]) * 0.5 * ((hhl[k+1][j][i2] + hhl[k][j][i2]) - (hhl[k+1][j][i2+1] + hhl[k][j][i2+1])) / ((hhl[k+1][j][i2] - hhl[k][j][i2]) + (hhl[k+1][j][i2+1] + hhl[k][j][i2+1]));
		}
	}
}
