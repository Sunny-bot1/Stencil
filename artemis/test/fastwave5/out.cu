...
__global__ void fastwave (double * __restrict__ d_ppgc, double * __restrict__ d_ppgk, int L, int M, int N) {
	...
	if (k >= k0 && k <= min(k0+blockDim.z-1, L-2) && j >= j0 && j <= min(j0+blockDim.y-1, M-1) && i >= i0 && i <= min(i0+blockDim.x-1, N-1)) {
		ppgc[k][j][i] = ppgk[k+1][j][i] - ppgk[k][j][i];
	}
}


extern "C" void host_code (double *h_ppgc, double *h_ppgk, int L, int M, int N) {
	...
	fastwave <<<gridconfig_1, blockconfig_1>>> (ppgc, ppgk, L, M, N);
