...
__global__ void fusion (double * __restrict__ d_ppgu0, double * __restrict__ d_ppgu1, double * __restrict__ d_ppuv, double * __restrict__ d_ppgc, double * __restrict__ d_hhl, int L, int M, int N) {
	...
	__shared__ double sh_ppuv[1024];
	__shared__ double sh_ppgc[1024];
	__shared__ double sh_hhl[1024];

	sh_ppuv[tid] = ppuv[k][j][i];
	//sh_ppgc[tid] = ppgc[k][j][i];
	//sh_hhl[tid] = hhl[k][j][i];
	__syncthreads();
	...
	if(tid < 512){
		...
		if (k >= k0 && k <= min(k0+blockDim_z-1, L-2) && j >= j0 && j <= min(j0+blockDim_y-1, M-1) && i >= i0 && i <= min(i0+blockDim_x-1, N-2)) {
			ppgu0[k][j][i] = (sh_ppuv[tid] - sh_ppuv[tid]) + (ppgc[k][j][i+1] + ppgc[k][j][i]) * 0.5 * ((hhl[k+1][j][i] + hhl[k][j][i]) - (hhl[k+1][j][i+1] + hhl[k][j][i+1])) / ((hhl[k+1][j][i] - hhl[k][j][i]) + (hhl[k+1][j][i+1] + hhl[k][j][i+1]));
			//ppgu0[k][j][i] = (ppuv[k][j][i+1] - sh_ppuv[tid]) + (ppgc[k][j][i+1] + sh_ppgc[tid]) * 0.5 * ((hhl[k+1][j][i] + sh_hhl[tid]) - (hhl[k+1][j][i+1] + hhl[k][j][i+1])) / ((hhl[k+1][j][i] - sh_hhl[tid]) + (hhl[k+1][j][i+1] + hhl[k][j][i+1]));
		}
	}
	else{
		double (*ppgu1)[MM][NN] = (double (*)[MM][NN]) d_ppgu1;
		if (k >= k0 && k <= min(k0+blockDim_z-1, L-2) && j >= j0 && j <= min(j0+blockDim_y-1, M-2) && i >= i0 && i <= min(i0+blockDim_x-1, N-1)) {
			ppgu1[k][j][i] = (ppuv[k][j+1][i] - sh_ppuv[tid]) + (ppgc[k][j+1][i] + ppgc[k][j][i]) * 0.5 * ((hhl[k+1][j][i] + hhl[k][j][i]) - (hhl[k+1][j+1][i] + hhl[k][j+1][i])) / ((hhl[k+1][j][i] - hhl[k][j][i]) + (hhl[k+1][j+1][i] + hhl[k][j+1][i]));
			//ppgu1[k][j][i] = (ppuv[k][j+1][i] - sh_ppuv[tid]) + (ppgc[k][j+1][i] + sh_ppgc[tid]) * 0.5 * ((hhl[k+1][j][i] + sh_hhl[tid]) - (hhl[k+1][j+1][i] + hhl[k][j+1][i])) / ((hhl[k+1][j][i] - sh_hhl[tid]) + (hhl[k+1][j+1][i] + hhl[k][j+1][i]));
		}
	}
}
